...
export function CompletedProjectsBlock({ rows, avgLifetimeDays }: CompletedProjectsBlockProps) {
  if (rows.length === 0) return null;

  const byMonth = new Map<string, UnifiedRow[]>();
  for (const r of rows) {
    const month = r["Closed month"] && r["Closed month"] !== "—" ? r["Closed month"] : "Date unavailable";
  ...
  const hasEstimatedCloseDate = rows.some((r) => (r.Closed ?? "—") === "—" && r["Last activity"] && r["Last activity"] !== "—");
  ...
  return (
    <div className="completed-block" style={{ marginTop: 36 }}>
      ...
      >
        Completed <span style={{ color: "#8fa1bf", fontWeight: 400, fontSize: "1rem" }}>· {rows.length}</span>
      </h2>
      {months.map((month, monthIdx) => {
        ...
        return (
          <div key={month}>
            ...
            {monthRows.map((r, i) => {
              const lifetime = r["Lifetime (days)"] === "—" ? null : parseInt(r["Lifetime (days)"], 10);
              const vsAvg =
              ...
              return (
                <div key={`${month}-${r.Project}-${r.Closed ?? r.Created}`}>
                  <div className="dashboard-row">
                    <div style={{ fontSize: "0.9rem", color: "#c4cde0", display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#9fb0cd" }}>
                      {(r.Closed ?? "—") !== "—"
                        ? r.Closed
                        : (r["Last activity"] && r["Last activity"] !== "—" ? `${r["Last activity"]}*` : "—")}
                    </span>
                    <span style={{ fontSize: "0.75rem", color: "#4ade80" }}>{r["Lifetime (days)"]}</span>
                    <span
                      style={{
                        fontSize: "0.75rem",
