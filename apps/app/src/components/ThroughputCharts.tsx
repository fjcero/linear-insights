...
export function ThroughputCharts({
  ...
  if (data.length === 0) {
    return (
      <p style={{ color: '#9fb0cd' }}>
        No project data by {granularity === 'WoW' ? 'week' : 'month'}.
      </p>
    )
  }
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 'auto', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.6875rem', color: '#9fb0cd' }}>
            ...
            {loading && (
              <span style={{ fontSize: '0.6875rem', color: '#9fb0cd' }}>Updating…</span>
            )}
          </div>
        ...
        <ResponsiveContainer width="100%" height={290}>
          {chartView === 0 ? (
            ...
            <ComposedChart data={data}>
              ...
              <Bar
                dataKey="closed"
                name={granularity === 'WoW' ? 'Closed / wk' : 'Closed / mo'}
                radius={[3, 3, 0, 0]}
              >
                {data.map((row) => (
                  <Cell
